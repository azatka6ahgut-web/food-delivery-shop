def test_admin_changed_status_order_true(api_client , auth_headers , admin_headers, second_user_headers):
    ...
    admin_change = api_client.patch(f"/orders/{order_id}/status", headers=admin_headers, json={"status": "delivered"},)

    assert admin_change.status_code == 200

    user_change = api_client.patch(f"/orders/{order_id}/status", headers=second_user_headers, json={"status": "delivered"},)
